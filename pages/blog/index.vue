<template>
  <main class="py-32 px-4 sm:px-8 lg:px-16 bg-primary-950 min-h-screen text-white">
    ...
    <div v-if="displayedPosts && displayedPosts.length" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 auto-rows-[250px]">
      <NuxtLink
      ...
      >
        ...
        <!-- Dark Overlay -->
        <div class="absolute inset-0 bg-black bg-opacity-50 group-hover:bg-opacity-60 transition-opacity duration-300 z-10"></div>
        <!-- Content -->
...
const { data: allPosts, error, pending } = await useAsyncData('all-blog-posts', () => 
  queryCollection('blog')
    .select('title', 'coverImage', 'publishDate', 'slug') 
