<template>
  <main class="py-32 px-4 sm:px-8 lg:px-16 bg-primary-950 min-h-screen text-white">
    ...
    <div v-if="displayedWorks && displayedWorks.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"> <!-- Responsive columns: 1 -> 2 -> 3 -->
      <!-- Render ProjectCard dynamically within the grid -->
      <div
        v-for="(project, index) in displayedWorks"
        :key="project._path"
...
<style scoped>
/* Ensure container takes up grid space */
.work-item-container {
  /* You might need styles here depending on grid behavior */
}
</style> 